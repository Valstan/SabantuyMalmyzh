-- Миграция: «Народная лента» PR3 — взаимодействие: submission_reactions (лайки),
-- submission_comments (комментарии, постмодерация), content_reports (жалобы →
-- авто-скрытие). Все НЕ versioned → без `_v` (обход G7). Реакции/комменты ссылаются
-- на submissions (FK submission_id ON DELETE set null — как drizzle для relationship).
-- Жалобы полиморфны (target_type+target_id, не Payload-relationship).
--
...
-- → создаёт эта миграция. Идемпотентно: enum/FK — DO/EXCEPTION, таблицы/индексы —
-- IF NOT EXISTS. Зеркало для payload migrate — 20260627_140000.ts.
