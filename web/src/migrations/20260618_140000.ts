...
/**
 * Коллекция quiz-questions (познавательная игра-«угадайка», директива brain
 * 2026-06-18). Версионируемая (drafts) + localized-подполя в массиве вариантов →
 * 8 таблиц (база/_locales/options/options_locales + их _v-зеркала) + 9 enum'ов +
 * связка payload_locked_documents_rels.quiz_questions_id.
 *
 * Таблицы НОВЫЕ → push на dev создал их без интерактива (G23 не грозит). Точные
 * имена/типы/индексы/FK сняты push-inspect'ом (pg_dump) из dev-БД. На проде push
 ...
 * (20260604_160000) — не пересоздаём. Зеркало для psql — 20260618_140000.sql.
 */
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
    ALTER TABLE "payload_locked_documents_rels" DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_quiz_questions_fk";
    ...
    DROP TABLE IF EXISTS "_quiz_questions_v_version_options_locales" CASCADE;
    DROP TABLE IF EXISTS "_quiz_questions_v_version_options" CASCADE;
    DROP TABLE IF EXISTS "_quiz_questions_v_locales" CASCADE;
    DROP TABLE IF EXISTS "_quiz_questions_v" CASCADE;
    ...
    DROP TYPE IF EXISTS "enum__quiz_questions_v_published_locale";
    DROP TYPE IF EXISTS "enum__quiz_questions_v_version_status";
    DROP TYPE IF EXISTS "enum__quiz_questions_v_version_difficulty";
    DROP TYPE IF EXISTS "enum__quiz_questions_v_version_format";
    DROP TYPE IF EXISTS "enum__quiz_questions_v_version_theme";
    DROP TYPE IF EXISTS "enum_quiz_questions_status";
    DROP TYPE IF EXISTS "enum_quiz_questions_difficulty";
    DROP TYPE IF EXISTS "enum_quiz_questions_format";
    DROP TYPE IF EXISTS "enum_quiz_questions_theme";
  `)
}
