 ...
 * объявления реальной даты. На сайте рядом с программой и обратным отсчётом
 * показывается обязательное предупреждение, что дата предварительная
 * (см. lib/festival.ts → FestivalNotice).
 *
 *   corepack pnpm -C web payload run src/seed/seedDemoProgram.ts
 *
 * Все события — на ОРИЕНТИРОВОЧНУЮ дату (последняя суббота июня, как в прошлые
 * годы; Сабантуй-2024 был 29 июня). Слаги начинаются с «demo-», чтобы их легко
 * было найти и удалить, когда организаторы заведут настоящие дату и программу.
 *
...
// Ориентировочная дата: суббота 27 июня 2026 (как в прошлые годы — конец июня).
