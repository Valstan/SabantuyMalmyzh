...
**Статус:** **deploy-smoke #011 расширен + SMTP-адаптер подключены и выкачены на ПРОД** (PR #23, #24); ранее праздничный фолк-редизайн (#21), localization + контент со старого WP (#17/#19/#20). Всё на проде, проверено вживую. Владелец завёл первого admin и привязал домен/TLS (#1, #2 ✅).
**Дата обновления:** 2026-06-05
**Веха:** M1 ✅ · M2 ✅ · усиление+фронт ✅ · M3 деплой ✅ · season-MVP «Программа»/«Карта» ✅ · двуязычие RU/TT ✅ (#17) · контент со старого WP ✅ на ПРОДЕ (#19/#20) · праздничный редизайн ✅ на ПРОДЕ (#21) · **deploy-smoke /+/map+/admin ✅ (#23) · SMTP-адаптер ✅ (#24)**. Дальше: владелец вписывает реальные SMTP-секреты в прод-env (код готов); наполнение контента (события, реальный текст «Политики ПДн»); публичный язык-тумблер (когда появятся реальные TT-переводы).
