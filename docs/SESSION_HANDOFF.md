...
2. **Первый admin на проде** (контент ✅ засеян 2026-06-05): зайти на `/admin` (экран create-first-user) и завести админа (пароль — свой, портер на проде админа НЕ создаёт). Прод-БД уже наполнена: 3 страницы (`o-sabantuy`/`kontakty`/`privacy`), 2 альбома галереи (13 фото, изображения отдаются). Осталось добавить **события** (с площадкой/тайм-слотами) и заменить **шаблонный текст «Политики ПДн»** реквизитами оператора.
