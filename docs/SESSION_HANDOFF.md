...
5. **Season-MVP #2 «Карта фестиваля»** — ✅ **построена** (глобал `festival-map`: план-картинка + объекты по типам; страница `/map` + nav). Asset больше **не блокер**: план = загрузка картинки в `/admin`, объекты = массив. Когда у владельца появится план — загружает, и карта наполняется. Миграция `20260604_140000` (таблицы `festival_map`/`festival_map_points` + enum) — на прод через `apply-migration.yml`.
...
7. Опц.: deprecation `actions/setup-node@v4` (Node 20) → бампнуть; фронт (hero-изображения событий через админку; фильтр по дням ✅ сделан); rate-limit при масштабе → Redis.
