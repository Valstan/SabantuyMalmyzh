...
**Память экосистемы одна — у Мозга (D-097, ADR-0013 мозга, 2026-09-18).** Читать **индекс → один файл записи**, никогда реестр целиком: `GOTCHAS.md`/`REFERENCE.md` — индексы, тела — `gotchas/G<N>.md`, `reference/R<N>.md`, идеи — `ideas/NNN-*.md`. Поиск по всей памяти одной командой:

```bash
python ../brain_matrica/.claude/scripts/brain_find.py "слова симптома или темы"
```

Не нашёл с первого запроса — это письмо brain, не молчание. Писать в память Мозга нельзя — только письмом (#009); у себя переносимое держим **одной строкой-указателем** на `G<N>`/`#NNN`, не копией (#087). Локально живёт оперативное (handoff, PENDING, карта кода) и специфичное для стека; локальный реестр > 50 КБ — на раскладку «индекс + файл на запись».
...
Наши гейты: `.github/workflows/gates.yml` (сканер секретов · сканер recon-литералов · typecheck · lint · **test** (vitest) · приёмка клиента vault — на каждый PR, required-контекст `main`) + `deps-audit.yml` (`pnpm audit --prod`, не required) + `build` в `deploy-prod.yml` после мержа. `lint` гоняется с `--max-warnings 0`: без этого `next lint` отдаёт exit 0 при предупреждениях, и «зелёный» перестаёт значить «нарушений нет».

**Три правила канона по D-096 (решение владельца 2026-09-18):**
- **#125 — все гейты гоняет CI, ни одного «руками».** Локальный прогон typecheck/lint/test — предпосылка, не гейт; гейт — тот, что виден в `gh pr checks`. Проверка, которую делает только агент на своей машине, гейтом не считается (ревизия #104).
- **#166 — фича на 3+ PR с новой границей** (роль, арендатор, внешний вход, демо-режим) получает **финальное ревью всей ветки целиком** до включения: дифф последнего PR не показывает, что делают первые два.
- **#152 — дата, названная в письме или в PENDING, в тот же ход попадает в будильник:** пункт PENDING со `state: active` и датой в тексте, либо `due: YYYY-MM-DD` в строке метаданных. Дата вне PENDING — не дата.
