...
🔍 **Аудит этих категорий грепом — только `rg`, никогда `grep -i` по кириллице** (G286). В Git Bash под Windows локаль `C`: `grep -i 'малмыж'` не находит `Малмыж`, а `grep -i 'МАЛМЫЖ'` отдаёт **пустой вывод** — неотличимый от чистого результата. Термины ищем в **обеих** формах письма: имя хостера, соседа и проекта живут в репо и латиницей, и кириллицей, и вычистка одной формы выглядит завершённой. Поймано на себе 2026-09-01: латинскую форму имени хостера вычистили 31.08, кириллическая пережила чистку в живом документе — тот же факт, та же категория, соседняя строка греп-лога, которой не было.
