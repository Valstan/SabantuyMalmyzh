...
- Деплой — **systemd + nginx + GitHub Actions**, Ubuntu 24.04. Адрес бокса, порт и имя пользователя в отслеживаемых файлах не держим (D-038): они живут в `secrets` репозитория, оттуда их берут воркфлоу. **SSH-алиас `sabantuy` держим сознательно** — это локальная метка в `~/.ssh/config` раннера, она равна имени репозитория и recon-нейтральна. ⚠️ Прежняя редакция этой строки обещала, что алиаса в отслеживаемых файлах нет — и была **неверна**: он лежал в 28 воркфлоу (проверка 2026-09-01). Утверждение исправлено, а не подогнано.
...
**Гейт:** сканер секретов в `gates.yml` (gitleaks, конфиг `.gitleaks.toml`) на каждый PR — история целиком (`fetch-depth: 0`), находка валит job (`--exit-code 1`), значение не печатается (`--redact`). Красный показан 2026-08-31 на своих правилах `vk-token-hex85` и `telegram-bot-token-anywhere`, оба — в URL-форме.
...
🔍 **Аудит этих категорий: пустой вывод сам по себе ничего не значит.** У обоих наших грепов есть слепое пятно, и оба молчат одинаково — `rc=1`, ноль строк, неотличимо от чистого результата.

| Инструмент | Слеп к | Проверено |
|---|---|---|
| `grep -i` | кириллице (локаль `C`): `grep -i 'МАЛМЫЖ'` → пусто при наличии строки | G286 |
| `rg` | шаблонам с ведущим `/`: MSYS конвертирует такой аргумент в windows-путь до передачи нативному `rg.exe` | 2026-09-01 |

Рабочие формы для путей: `MSYS_NO_PATHCONV=1 rg -F '/etc/foo'` · `rg -F 'etc/foo'` (без ведущего слеша) · `rg '[/]etc/foo'`. Трюк с `//` **не работает** — проверено.

> **Правило, которое переживёт следующий инструмент: метод проверяется на заведомо непустом контроле.** Прежде чем поверить пустому выводу — прогони тот же шаблон тем же инструментом по строке, которая точно есть. Не совпало — слепо не оружие, а ты сам. Это ловит и оба известных пятна, и те, которых мы ещё не нашли.
...
⚠️ Оба пятна пойманы на себе 2026-09-01, в один час: латинскую форму имени хостера вычистили 31.08, а кириллическую `grep -i` не показал; следом `rg` вернул ноль по серверным путям, и на этом ложном «чисто» было **опубликовано утверждение, что путей в репо нет**. Смена инструмента не лечит класс — она меняет слепое пятно.
