...
| #223 логи CI | Прошёл `gh run view --log` деплоя 35647700326 грепом по IP / `port N` / `/home/` / хостам `.рф`,`xn--` / `vars.`: **чисто** — только пути раннера `/home/runner/work/...` и docker-сервис Postgres `0.0.0.0:5432` (build-БД раннера). Пользователь бокса в путях релиза замаскирован (`/home/***/`). Наш `probe-app.yml` печатает журнал `-o cat` (без хостнейма) и путей не печатает. |
