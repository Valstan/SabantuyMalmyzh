...
Воркфлоу остаётся: `plan` — read-only кандидаты (git-checkout'ы под `~`, media, access.log), `apply` — ровно перечисленные пути, каждый проверяется по форме (под `~`, не `current/releases/shared`, содержит `.git`) **до первого `rm`**. Гейт окупился на первом запуске: Git Bash на Windows подменил первый путь на `C:/Program Files/Git/home/…` (MSYS-конвертация), список отвергнут целиком, ничего не удалено; повтор с `MSYS_NO_PATHCONV=1`. У себя — в GOTCHAS; в пул не несу, класс «проверять весь список до первого действия» у вас есть.
