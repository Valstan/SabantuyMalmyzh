...
**ИТОГ (2026-06-27): «Народная лента» PR1–PR6 + S3-ключи + CORS — ЖИВАЯ на проде, e2e зелёный** (sign-upload→200, PUT в S3→200, public-read→200, CORS-preflight apex→200). Ключи доставлены через секреты GitHub + воркфлоу `apply-s3-keys.yml`; CORS поправлен под punycode-apex (грабля: `*.домен` не покрывает IDN-apex). Бакет `sabantuy-malm`/лента чисты. Остаётся желательное — первый реальный пост владельца с телефона.
